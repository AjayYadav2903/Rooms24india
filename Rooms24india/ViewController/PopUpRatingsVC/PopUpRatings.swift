...
class PopUpRatings: UIView , UIGestureRecognizerDelegate {
...
//    }
    ...
    func loadViewFromNib() {
        let nib = UINib(nibName: "PopUpRatings", bundle: Bundle(for: type(of: self)))
...
//        window?.bringSubview(toFront: self)
    ...
    class func showRequestPopup(strMgs:String, strTitle:String, strActionTitle:String? = nil, isShowCloseButton:Bool = true, isRemoveAllSubview:Bool = false , acceptBlock: (() -> Void)? = nil, rejectBlock: (() -> Void)? = nil){
        DispatchQueue.main.async {
            let view = PopUpRatings(frame: UIScreen.main.bounds)
